...
| **Verification step to prevent bot activity** | JWT auth + slowapi rate limiting + bcrypt 5-attempt lockout |
...
- **3:30–5:00 — Topic-bullet checklist, narrated.** Search + **category sort**, **reviews & ratings**, **sort-by-rating**, **favorites**, **deals/coupons** — saying each name. Then type garbage into a field to show **validation** rejecting it gracefully, and trigger the **account lockout** to show **bot prevention**. *(Functionality + validation + topic coverage)*
